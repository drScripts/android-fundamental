...
object HeroData {
    private val heroNames = arrayOf<String>(
        "Ahmad Dahlan",
        "Ahmad Yani",
        "Sutomo",
        "Gatot Soebroto",
    ...
    val dataList : ArrayList<Hero>
    // memconfert data ke model Hero
    get(){
        val list = arrayListOf<Hero>()
        for (positions in heroNames.indices){
            val hero = Hero()
            hero.name = heroNames[positions]
            hero.detail = heroDetails[positions]
            hero.photo = heroImage[positions]
            list.add(hero)
        }
        return list
    }
