...
object Heroes {
    ...
    private val heroDescription = arrayOf(
    ...
    val dataList : ArrayList<HeroModel>
    get() {
        val list = arrayListOf<HeroModel>()
        for (position in heroName.indices){
            val heroModel = HeroModel()
            heroModel.name = heroName[position]
            heroModel.description = heroDescription[position]
            heroModel.image = heroImage[position]
            list.add(heroModel)
        }
        return list
    }
